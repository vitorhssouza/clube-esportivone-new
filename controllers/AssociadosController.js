...
class AssociadosController{
    ...
    static async agendamento(req, res){
        ...
        // const user = await Associados.findOne({
        //     raw: true,
        //     where: {id_associados: id}
        // })
        // // console.log(user)
        ...
        const areas = area_comum.map((resultado) => resultado.get({
            plain:true
        }))
        ...
        res.render('menu_associados/agendamento', {id, areas});
    }
    ...
    static alterarSenha(req, res){
        res.render('troca_senha/trocar_senha')
    }

    static async alterarSenhaSalvar(req, res){
        const email = req.body.email;
        const senha = req.body.password;
        const confirmaSenha = req.body.confirmPassword
        ...
        if(senha != confirmaSenha){
            ...
            req.flash('error', 'Senhas incorretas')
            req.session.save(() =>{
                res.redirect(`/clube/alterarSenha`)
            });
        }else{
            if(associado === null){ 
                req.flash('message', 'postagem');
                req.flash('error', 'Email não está cadastrado')
                req.session.save(() =>{
                    res.redirect(`/clube/alterarSenha`)
                })
                ...
                await Associados.update({senha: senhaCriptografada}, {where: {email: email}})
